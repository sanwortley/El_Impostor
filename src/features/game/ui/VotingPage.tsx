...
import { User, ShieldAlert, CheckCircle2, FastForward, Clock, Mic, MicOff } from 'lucide-react';
...
export const VotingPage: React.FC = () => {
    const { players, localPlayer, castVote, skipVoting, currentVotes, timeRemaining, tickTimer, gameMode, settings, speakingPlayers } = useGameStore();
    ...
    return (
        ...
        >
            ...
            {/* Voice Chat Status during Voting - Discord Style Scalable */}
            {settings.voiceChat && (
                <div className="w-full mt-2 pt-4 border-t border-white/5">
                    ...
                    <div className={`grid gap-3 ${players.length <= 8 ? 'grid-cols-4' :
                        players.length <= 15 ? 'grid-cols-5' :
                            players.length <= 24 ? 'grid-cols-6' : 'grid-cols-7'
                        }`}>
                        {players.map((p, idx) => {
